...
############
    ...
    masterEntries = []
    tempdata = [(random.randint(1,19)) for i in range(30000000)]    
    #print tempdata
    npdata = numpy.array(tempdata).reshape(100,300000)
    #print len(npdata)
    lookupTable = [(random.randint(0,255)) for i in range(20)]    
	...
	for i in range(len(data)):
	    result.append(lookupTable[data[i]])
        return result
        


    requests = makeRequests(processList, npdata, combineResults)
